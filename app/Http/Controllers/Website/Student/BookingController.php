...
class BookingController extends Controller
{
    ...
    public function bookinglistPagination(Request $request){        
        ...
        )->where('b.student_id','=',$looged_studentid)->where('b.status','=','Pending');
        ...
        if(!empty($getBookings) && count($getBookings) > 0){
            if($getBookings->total() >= $getBookings->currentPage()){
                foreach($getBookings as $getBooking){                          
                    $returnData['html'] .= view('/student/booking-listing-rec',
                        [                            
    ...
    public function bookinglistDetails(Request $request){        
        ...
        if(!empty($getBookings) && count($getBookings) > 0){
            foreach($getBookings as $getBooking){                    
                $returnData['html'] .= view('/student/booking-listing-rec-details',
                    [                            
                        'booking_id'=>$getBooking->booking_id,
                        'tutor_id'=>$getBooking->tutor_id,
                        'booking_date'=>$getBooking->booking_date,

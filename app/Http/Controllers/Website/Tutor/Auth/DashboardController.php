...
class DashboardController extends Controller
{
    ...
    {       
        ...
        $totalStudentBookings = DB::table('booking AS b')->select('b.id')->where('b.tutor_id','=',$logged_tutor_id)->count();

        $q_totalEarn         = DB::table('booking AS b')->select('b.id',DB::raw('SUM(tutor_price) as total_tutor_earn'))
        ->where('b.tutor_id','=',$logged_tutor_id)->where('b.status','!=','Rejected')->where('b.status','!=','Cancel')->where('b.status','!=','Missed')->limit(1)->get();
        
        if(!empty($q_totalEarn)){
            foreach($q_totalEarn as $value){
                $totalEarn = $value->total_tutor_earn;
            }
        }
        else{
            $totalEarn         =0;
        }
        $totalAttendHours   = DB::table('booking AS b')->select('b.id')->where('b.tutor_id','=',$logged_tutor_id)->where('b.status','=','Completed')->count();
        return view('/tutor/tutor-dashboard')->with(compact("totalStudentBookings","totalEarn","totalAttendHours"));        
    }
    ...
    public function dashboardlistPagination(Request $request){   
        ...
        )->where('b.tutor_id','=',$logged_tutor_id)->where('b.booking_date','=',date("Y-m-d"));
        ...
        if(!empty($getBookings) && count($getBookings) > 0){
            if($getBookings->total() >= $getBookings->currentPage()){
                foreach($getBookings as $getBooking){                                              
                    $returnData['html'] .= view('/tutor/tutor-dashboard-rec',
                        [                            
    ...
    public function odashboardlistDetails(Request $request){        
        $logged_tutor_id       =  Auth::guard('tutors')->id();
        ...
        if(!empty($getBookings) && count($getBookings) > 0){
            foreach($getBookings as $getBooking){                    
                $returnData['html'] .= view('/tutor/tutor-dashboard-rec-details',
                    [                            
                        'booking_id'=>$getBooking->booking_id,
                        'student_id'=>$getBooking->student_id,
                        'booking_date'=>$getBooking->booking_date,
    ...
    public function dashboardChangeStatus(Request $request)
    {
        $logged_tutor_id       =  Auth::guard('tutors')->id();
